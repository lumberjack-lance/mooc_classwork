...
public class TheGame extends GameThread{
    ...
    //The speed for the moving sad ball
    private float mSadSpeedX = 0;
    ...
    //Will store the image of the Temporary ball (score ball)
    ...
    public TheGame(GameView gameView) {
        ...
        //Prepare the image of the SmileyBall so we can draw it on the screen (using a canvas)
        mTempBall=  BitmapFactory.decodeResource
                (gameView.getContext().getResources(),
                        R.drawable.smiley_ball);
    ...
    public void setupBeginning() {
        ...
        mSadBallY[0] = mCanvasHeight / 2;

        mSadBallX[1] = mCanvasWidth - mCanvasWidth/ 3;
        mSadBallY[1] = mCanvasHeight / 2;

        mSadBallX[2] = mCanvasWidth / 2;
        mSadBallY[2] = mCanvasHeight / 4;

        //Initialize sad ball 2's speed
        mSadSpeedX = mCanvasWidth / 3;
    ...
    protected void updateGame(float secondsElapsed) {
        ...
        //Get the sad ball 2 moving
        mSadBallX[2] = mSadBallX[2] + secondsElapsed * mSadSpeedX;
        ...
        // Check if the sad ball leaves the screen
        if((mSadBallX[2] <= mSadBall.getWidth() / 2 && mSadSpeedX < 0) || (mSadBallX[2] >= mCanvasWidth - mSadBall.getWidth() / 2 && mSadSpeedX > 0) ) {
            mSadSpeedX = -mSadSpeedX;
        }
        ...
        //Check for SadBall collision
        //Check for SmileyBall collision
        if(updateBallCollision(mSadBallX[2], mSadBallY[2])) {
            //Increase score
            updateScore(-1);
            //Reset Paddle
            ResetPaddle();
            //Make The Smiley Ball Frown
            Frown();
        }
    ...
    public void Frown(){
        mSmileyBall = mSadBall;

    }

    public void Smile(){
        mSmileyBall = mTempBall;
    }
